...
_PAGE_FIXTURE_KEYS = ("page", "open_login_page", "login_successfully", "product_page", "cart_page", "checkout_page")


def _launch_browser(playwright, browser_name: str, headless: bool, channel: str | None):
    launchers = {
        "chromium": lambda: playwright.chromium.launch(
            headless=headless,
    ...
    launcher = launchers.get(browser_name)
    if launcher is None:
...
def _login_with_env(login_page: LoginPage):
    login_page.login(
...
def page():
    ...
    with sync_playwright() as playwright:
        browser = _launch_browser(playwright, browser_name, headless, channel)
...
def login_successfully(open_login_page):
    _login_with_env(open_login_page)
